...
def cria_gerador(b, s):
    """
    Esta função cria um gerador (usando dois numeros para bits e seed/estado inicial) e verifica a validade dos argumentos fornecidos.
    --------------------------
    {int x int} -> {"gerador"}
    """
    if not isinstance(b, int) or (b != 32 and b != 64) or not isinstance(s, int) or not 0 < s < 2**b:
        raise ValueError("cria_gerador: argumentos invalidos")
    return {"b": b, "s": s} # Gerador no formato de um dicionario, sendo b bits e s a seed.
...
def atualiza_estado(g):
    """
    Esta função atualiza o estado ("seed") de um gerador, de acordo com o algoritmo xorshift de geração de números pseudoaleatórios, e devolve-o.
    --------------------
    {"gerador"} -> {int}
    """
    s = obtem_estado(g)
...
def eh_gerador(g):
    ...
    {universal} -> {bool}
    """
    return isinstance(g, dict) and len(g) == 2 and "b" in g and "s" in g and type(obtem_estado(g)) == type(g["b"]) == int
...
def gera_numero_aleatorio(g, n):
    """
    Esta função atualiza o estado do gerador e devolve um número aleatório no intervalo de [1,n] obtido a partir do novo estado da seed do gerador.
    --------------------------
    {"gerador" x int} -> {int}
    """
...
def gera_carater_aleatorio(g, c):
    """
    Esta função atualiza o estado do gerador e devolve um caracter aleatório da lista de letras de A até essa letra, obtido a partir do novo estado da seed do gerador.
    --------------------------
    {"gerador" x str} -> {str}
    """
...
def cria_coordenada(col, lin):
    """
    Esta função cria uma coordenada (usando uma letra e um numero para a coluna e a linha, respetivamente) e verifica a validade dos argumentos fornecidos.
    --------------------------
    ...
    return {"col": col, "lin": lin} # Coordenadas do tipo de dicionario em que a chave col representa a coluna e lin a linha.
...
def eh_coordenada(arg):
    ...
    return isinstance(arg, dict) and len(arg) == 2 and "col" in arg and "lin" in arg and isinstance(obtem_coluna(arg), str) and isinstance(obtem_linha(arg), int) and 99 <= obtem_linha(arg) <= 1 and ord("A") <= obtem_coluna(arg) <= ord("Z")
...
def obtem_coordenadas_vizinhas(c):
    """
    Esta função recebe uma coordenada e devolve as coordenadas vizinhas, começando pela coordenada da diagonal superior-esqueda e seguinte o sentido horário.
    -------------------------
    {"coordenada"} -> {tuple}
    """
...
def obtem_coordenada_aleatoria(c, g):
    """
    Esta função recebe uma coordenada e devolve uma coordenada aleatoria em que a coordenada fornecida define o tamanho máximo da coluna e da linha.
    --------------------------------------------
...
def cria_parcela():
    ...
    return {"estado": "tapada", "marcada": False, "mina": False} # Parcela no formato de dicionário com as propriedades necessárias
...
def eh_parcela(p):
    ...
    return type(p) == dict and len(p) == 2 and "mina" in p and "estado" in p and type(p["mina"]) == bool and p["estado"] in ("tapada", "limpa", "marcada")
...
def parcela_para_str(p):
    """
    Esta função devolve a cadeia de caracteres que representa a parcela em funcao do seu estado: parcelas tapadas (#), parcelas marcadas (@), parcelas limpas sem mina (?) e parcelas limpas com mina (X).
    --------------------
    {"parcela"} -> {str}
    """
...
def alterna_bandeira(p):
    """
    Esta função recebe uma parcela p e modifica-a destrutivamente (desmarca se estiver marcada e marca se estiver tapada, devolvendo True. Em qualquer outro caso, nao modifica a parcela e devolve False.)
    ---------------------
    {"parcela"} -> {bool}
    """
...
def cria_campo(col, lin):
    """
    Esta função cria um campo (usando uma letra e um numero para o tamanho maximo de colunas e de linhas, respetivamente) e verifica a validade dos argumentos fornecidos.
    ------------------------
    {str x int} -> {"campo"}
    """

    if not isinstance(lin, int) or not isinstance(col, str) or not 99 >= lin >= 1 or not ord("Z") >= ord(col) >= ord("A"):
    ...
    for i in range(1, lin+1):
        for ii in range(ord('A'), ord(col)+1):
            d["coords"][coordenada_para_str(cria_coordenada(chr(ii), i))] = cria_parcela() # Vai criando as parcelas para X coordenada
    return d
...
def obtem_coordenadas(c, s):
    ...
    for cc in coordenadas:
        ...
        if (s == "limpas" and eh_parcela_limpa(parcela)) or (s == "marcadas" and eh_parcela_marcada(parcela)) or (s == "minadas" and eh_parcela_minada(parcela)) or (s == "tapadas" and eh_parcela_tapada(parcela)): # "Filtra" as coordenadas de forma a retornar aquelas do valor s que queremos.
...
def obtem_numero_minas_vizinhas(campo, coord):
    """
    Esta função devolve o numero de parcelas vizinhas da parcela na coordenada coord que escondem uma mina.
    ---------------------------------
    {"campo" x "coordenada"} -> {int}
    """
    vizinhas = obtem_coordenadas_vizinhas(coord)
    minadas = obtem_coordenadas(campo, "minadas")
    return len(tuple(filter(lambda x: x in vizinhas, minadas))) # Retorna o tamanho da lista das coordenadas que são vizinhas e minadas
...
def eh_campo(c):
    ...
    if not (isinstance(c, dict) and len(c) == 3 and "col" in c and "lin" in c and "coords" in c and isinstance(c["coords"], dict) and isinstance(obtem_ultima_linha(c), int) and isinstance(obtem_ultima_coluna(c), str) and ord("Z") >= ord(c["coords"]) >= ord("A") and 99 >= obtem_ultima_linha(c) > 0):
        return False
...
def eh_coordenada_campo(campo, coord):
    """
...
def campo_para_str(campo):
    ...
    linhas = [""] * (obtem_ultima_linha(campo) + 3) # Lista com o numero de linhas + 3 (para listar as colunas, e outras duas para as linhas da tabela)
    ...
    for i in range(1, obtem_ultima_linha(campo) + 1):
        linhas[i+1] += f"{i}|" if i >= 10 else f"0{i}|" # Se o número for inferior a 10 acrescenta um 0 antes
        num = (ord(col) - ord("A")) + 1
        for e in coords[(i-1) * num : i * num]: # Vê os elementos de uma linha do campo, sendo e -> (coordenada em string, parcela)
            coord, parcela = str_para_coordenada(e[0]), e[1]
            pps = parcela_para_str(parcela)

            if eh_parcela_limpa(parcela) and not eh_parcela_minada(parcela) and not eh_parcela_marcada(parcela): # No caso de não ter mina, ser uma parcela limpa e que não esteja marcada
                n_minas = obtem_numero_minas_vizinhas(campo, coord)
                pps = str(n_minas) if n_minas != 0 else ' ' # Indica o número de minas à sua volta, se for 0 então não dirá nada
                linhas[i+1] += pps
                continue
...
def coloca_minas(campo, coord, gerador, numero):
    """
    Esta função modifica o campo de forma a esconder um numero de minas, aleatoriamente, através do gerador, de forma a que as minas não sejam a coordenada indicada, estejam na sua vizinhança ou já estejam minadas.
    -------------------------------------------------------
    ...
    while numero > i:
        ...
        if random == coord or coord in vizinhas or random in vizinhas or (random_s in campo["coords"] and eh_parcela_minada(campo["coords"][random_s])) or not random_s in campo["coords"]: # Se não resepeitar as condições para ser uma mina no campo, continua à procura de uma coordenada que satisfaça a condição.
            continue
        campo["coords"][random_s] = esconde_mina(campo["coords"][coordenada_para_str(random)])
        i += 1
...
def limpa_campo(campo, coord):
    """
    Esta função modifica o campo limpando a parcela na coordenada coord e devolvendo-a. Se nao houver nenhuma mina vizinha escondida, limpa iterativamente todas as parcelas vizinhas tapadas
    -------------------------------------
    ...
    coords = campo["coords"]
    vizinhas = obtem_coordenadas_vizinhas(coord)
    coords[coord_s] = limpa_parcela(coords[coord_s])
    ...
    for v in vizinhas:
        ...
        if eh_coordenada_campo(campo, v) and eh_parcela_tapada(coords[v_str]):
                dicionario[v_str] = limpa_parcela(coords[v_str])

    coords.update(dicionario) # Atualiza o campo com o dicionário de parcelas limpas

    for v in dicionario.keys():
        v = str_para_coordenada(v)
        limpa_campo(campo, v) # Continua a limpar as vizinhas e por ai a diante
    return campo
...
def jogo_ganho(campo):
    """
    Esta função recebe um campo do jogo das minas e devolve True se todas as parcelas sem minas se encontram limpas, ou False caso contrario.
    -------------------
    {"campo"} -> {bool}
    """
    ...
    return (len(coords) - len(minados)) == len(limpo)


def turno_jogador(campo):
    """
    Esta função recebe um campo de minas e oferece ao jogador a opcao de escolher uma acao (Limpar ou Marcar) e uma coordenada. Caso o jogador escolha limpar e limpe uma parcela que continha uma mina, então retorna False, caso contrário retorna True.
    -------------------
    {"campo"} -> {bool}
    """
    ...
    while not coord or not len(coord) == 3 or not str_para_coordenada(coord) or not eh_coordenada_campo(campo, str_para_coordenada(coord)):
        coord = input("Escolha uma coordenada:")

    if acao == "L":
        if eh_parcela_minada(campo["coords"][coord]):
            limpa_parcela(obtem_parcela(campo, str_para_coordenada(coord)))
            return False
        ...
        alterna_bandeira(campo["coords"][coord])
    return True


def minas(coluna, linha, minas, bits, seed):
    """
    A função minas, sendo a função principal que permite de facto jogar o jogo das minas, é uma função que recebe os argumentos necessários para criar um campo, um gerador e o número de minas a colocar no campo. A função devolve True caso o jogador ganhe o jogo e False caso perca.
    ---------------------------------------
    {str x int x int x int x int} -> {bool}
    """

    if not isinstance(coluna, str) or not ord("A") <= ord(coluna) <= ord("Z") or not isinstance(linha, int) or not 1 <= linha <= 99 or not isinstance(bits, int) or bits not in (32, 64) or not isinstance(seed, int) or seed == 0:
