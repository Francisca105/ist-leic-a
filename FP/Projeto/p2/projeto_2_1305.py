...
def cria_coordenada(col, lin):
    ...
    if not isinstance(lin, int) or not isinstance(col, str) or not 99 >= lin >= 1 or len(col) > 1 or not 90 >= ord(col) >= 65:
...
def eh_coordenada(arg):
    ...
    return isinstance(arg, dict) and len(arg) == 2 and "col" in arg and "lin" in arg and isinstance(obtem_coluna(arg), str) and isinstance(obtem_linha(arg), int) and 99 >= obtem_linha(arg) >= 1 and ord("A") <= ord(obtem_coluna(arg)) <= ord("Z")
...
def eh_parcela(p):
    ...
    return type(p) == dict and len(p) == 3 and "mina" in p and "estado" in p and "marcada" in p and type(p["mina"]) == bool and p["estado"] in ("tapada", "limpa") and type(p["marcada"]) == bool
...
def cria_campo(col, lin):
    ...
    if not isinstance(lin, int) or not isinstance(col, str) or not 99 >= lin >= 1 or len(col) > 1 or not ord("Z") >= ord(col) >= ord("A"):
...
def obtem_coordenadas(c, s):
    ...
    for cc in coordenadas:
        ...
        if (s == "limpas" and eh_parcela_limpa(parcela)) or (s == "marcadas" and eh_parcela_marcada(parcela)) or (s == "minadas" and eh_parcela_minada(parcela)) or (s == "tapadas" and eh_parcela_tapada(parcela)):
...
def eh_campo(c):
    ...
    if not (isinstance(c, dict) and len(c) == 3 and "col" in c and "lin" in c and "coords" in c and isinstance(c["coords"], dict) and isinstance(obtem_ultima_linha(c), int) and isinstance(obtem_ultima_coluna(c), str) and ord("Z") >= ord(obtem_ultima_coluna(c)) >= ord("A") and 99 >= obtem_ultima_linha(c) > 0):
        return False

    for p in c["coords"].values():
        if not eh_parcela(p):
            return False
...
def campo_para_str(campo):
    ...
    for i in range(1, obtem_ultima_linha(campo) + 1):
        ...
        for e in coords[(i-1) * num: i * num]:
            ...
            if eh_parcela_limpa(parcela) and not eh_parcela_minada(parcela) and not eh_parcela_marcada(parcela):
...
def coloca_minas(campo, coord, gerador, numero):
    ...
    while numero > i:
        ...
        if random == coord or coord in vizinhas or random in vizinhas or (random_s in campo["coords"] and eh_parcela_minada(campo["coords"][random_s])) or not random_s in campo["coords"]:
            continue
...
def limpa_campo(campo, coord):
    ...
    for v in vizinhas:
        ...
        if eh_coordenada_do_campo(campo, v) and eh_parcela_tapada(coords[v_str]):
...
def turno_jogador(campo):
    ...
    while not coord or not len(coord) == 3 or not ord("A") <= ord(coord[0]) <= ord("Z") or not coord[1:].isdigit() or len(coord[1:]) > 2 or len(coord[1:]) == 1 or not 1 <= int(coord[1:]) <= 99 or not str_para_coordenada(coord) or not eh_coordenada_do_campo(campo, str_para_coordenada(coord)):
...
def minas(coluna, linha, minas, bits, seed):
    ...
    if not isinstance(coluna, str) or len(coluna) > 1 or not ord("A") <= ord(coluna) <= ord("Z") or not isinstance(linha, int) or not 1 <= linha <= 99 or not isinstance(bits, int) or bits not in (32, 64) or not isinstance(seed, int) or seed == 0 or not isinstance(minas, int) or minas <= 0 or minas > ((ord(coluna) - ord("A") + 1) * linha - 1) or (ord(coluna) < ord("C") and linha < 3):
