...
def parcelas_iguais(p1, p2):
    ...
    """
    return eh_parcela_tapada(p1) == eh_parcela_tapada(p2) and \
        eh_parcela_limpa(p1) == eh_parcela_limpa(p2) and \
        eh_parcela_marcada(p1) == eh_parcela_marcada(p2) and \
    ...
    for c in m["coords"].items():
        try:
            ...
            if not eh_parcela_tapada(parcela) or eh_parcela_marcada(parcela) or eh_parcela_minada(parcela):
                copy["coords"][coordenada_para_str(
                    str_para_coordenada(coord))] = cria_copia_parcela(parcela)
        except:
            continue
...
def campos_iguais(c1, c2):
    ...
    for coord in c1["coords"]:
        if not parcelas_iguais(obtem_parcela(c1, coordenada_para_str(coord)), obtem_parcela(c1, coordenada_para_str(coord))):
            return False
...
def jogo_ganho(campo):
    ...
    parcelas = (ord(obtem_ultima_coluna(campo)) -
                ord("A") + 1) * obtem_ultima_linha(campo)
